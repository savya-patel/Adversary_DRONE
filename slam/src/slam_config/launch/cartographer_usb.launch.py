"""
Launch Cartographer 2D SLAM with a SICK TiM USB LiDAR.

This launch starts:
- `sick_tim_usb_ros2/usb_lidar_node` publishing `sensor_msgs/LaserScan` on `/scan`.
- `tf2_ros/static_transform_publisher` for `base_link` -> `laser_frame`.
- `cartographer_ros/cartographer_node` configured via `config/cartographer_2d_lidar.lua`.
- `cartographer_ros/cartographer_occupancy_grid_node` publishing `/map`.

Odometry input is not required; Cartographer performs scan matching.
"""
...
def generate_launch_description():
    ...
    usb_lidar_node = Node(
        ...
        parameters=[{
            ...
            'range_min': 0.1,
            'range_max': 10.0,
            # Publish ~270 points across 270 degrees (~1 deg increment)
            'publish_points': 270,
            'log_throttle_sec': 2.0,
        }]
    )

    # base_link -> laser_frame static transform (no positional offset by default)
