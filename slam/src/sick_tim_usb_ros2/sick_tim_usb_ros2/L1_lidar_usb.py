"""
SICK TiM USB reader: serial CDC-ACM and PyUSB bulk transport.

- Initializes device via SOPAS commands: SetAccessMode, LMCstartmeas, Run,
  and enables LMDscandata streaming.
- Parses LMDscandata telegrams to distance arrays in meters.
- Designed for reuse by ROS2 nodes and CLI testing.
"""
...
class Lidar():
    def __init__(self, port: str | None = None, angular_resolution: str = ANGULAR_RESOLUTION, prefer_usb: bool = True):
        # Allow explicit serial port and probing fallback; prefer PyUSB when available.
        self.port = port
    ...
    def connect(self):
        """Open transport and initialize for streaming.

        Preference order:
        1) Explicit serial `self.port` if provided
        2) PyUSB bulk (VID:PID 19a2:5001) if available and `prefer_usb`
        3) Serial probe `/dev/serial/by-id/*` then `/dev/ttyACM*` `/dev/ttyUSB*`
        """
        # 1) Explicit serial path first if user supplied a port (currently disabled in this file)
        ...
        # 3) If USB not available, consider probing serial ports (disabled here).
