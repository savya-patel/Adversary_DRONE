...
# # --- Wait until vehicle is ready for arming ---
...
def send_body_velocity(vehicle, vx, vy, vz, yaw_rate):
    ...
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        ...
        0b010111000111, # type_mask (0x57: velocity + yaw_rate)
...
# get bounding box corners, center, and print center coordinates
 ...
 # end revision (9/29)
                    ...
                    kx = 0.001  # how fast drone YAWS to center target horizontally
                    ky = 0.001  # how fast drone moves UP/DOWN to center target vertically
                    kz = 0.002  # how fast drone moves FORWARD/BACKWARD to keep target at desired distance
                    
                    #simple proportional control (tune gains)
                    vx = -kz * err_z        # forward/backward (pitch)
                        # if too close = -vx
                        # if too far = +vx
                    vy = 0                  # lateral (roll)
                    vz = ky * err_y        # up/down (m/s) (throttle)
                        # if det drone below center, drone goes down = +vz
                        # if det drone above center, drone goes up = -vz
                    yaw_rate = kx * err_x   # rotate left/right (rad/s) (yaw)
                        # if det drone to the right = +yaw_rate (CW)
                        # if det drone to the left = -yaw_rate (CCW)

                    # prints drone movement direction
                    direction = []
                    if vx > 0.05: direction.append("forward")
                    elif vx < -0.05: direction.append("backward")
                    if vz > 0.05: direction.append("down")
                    elif vz < -0.05: direction.append("up")
                    if yaw_rate > 0.05: direction.append("yawing right")
                    elif yaw_rate < -0.05: direction.append("yawing left")
                    action_text = "Drone moving " + " + ".join(direction) if direction else "Drone holding position"
                    # Ex: Drone moving forward (0.12 m/s) + yawing left (0.03 rad/s)
                    print(f"{action_text} | vx={vx:.2f} m/s, vz={vz:.2f} m/s, yaw_rate={yaw_rate:.2f} rad/s")
                    cv2.putText(im0, action_text, (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,255), 2)
                

                    #send to cube
                    send_body_velocity(vehicle, vx, vy, vz, yaw_rate)
            ...
            if view_img:
                ...
                # ****Revision: q to quit, shows stream instead of first frame
