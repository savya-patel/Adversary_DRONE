...
Usage:
    ...
    $ python detect_test.py --weights best.pt --source 0 --max-det 1 --imgsz 320 --nosave --device 0 --half
"""
...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    #^^^turned off to send display to laptop instead
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        #print(f'Input tensor device: {img.device}') used to find if yolo is running cpu or gpu
        pred = model(img, augment=augment)[0]
        #print(f'Output tensor device: {pred.device}')
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            x_center, y_center = frame_w // 2, frame_h // 2  # Default to frame center
...
# get bounding box corners, center, and print center coordinates
                    ...
                    if conf >= 0.5:
                        ...
                        cv2.circle(im0, (x_center, y_center), 5, dot_color, -1)  # mark center with confidence-based color

                        if draw:  # Add bbox to image when drawing is enabled
                    ...
                    # Close the confidence check if block
                    if conf >= 0.5:
                        pass
            ...
            if frame_callback is not None:
                ...
                # ***q to quit, shows stream instead of first frame
